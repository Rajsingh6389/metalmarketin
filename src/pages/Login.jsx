...
export default function Login() {
  ...
  // helper to update context quickly (for direct login flow)
      ...
      <div className="w-full max-w-md bg-gray-900/70 backdrop-blur-md p-8 rounded-3xl shadow-neon border border-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="Email" type="email"
            className="w-full px-4 py-3 rounded-lg bg-gray-800 text-white focus:outline-none" required />
        ...
        <p className="mt-4 text-center text-gray-400">No account? <Link to="/signup" className="text-accent">Sign up</Link></p>
      </div>
    </div>
  );
}
